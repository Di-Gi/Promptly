...
export async function activate(context: ExtensionContext) {
    ...
    promptDecoration = window.createTextEditorDecorationType({
        color: GREEN_COLOR
    });

    let startChatDisposable = commands.registerCommand('promptly.startChat', async () => {
        await handleChat();
    });

    let extractCodeDisposable = commands.registerCommand('promptly.extractCode', async () => {
        await extractCodeFromLastResponse();
    });

    let startChatNotebookDisposable = commands.registerCommand('promptly.startChatNotebook', async () => {
        await handleChatNotebook();
    });

    let setupLocalModelDisposable = commands.registerCommand('promptly.setupLocalModel', async () => {
        await startLocalModelSetup();
    });

    let handleTracebackErrorDisposable = vscode.commands.registerCommand('promptly.handleTracebackError', async () => {
    ...
    let enterCommandModeDisposable = commands.registerCommand('promptly.enterCommandMode', async () => {
    ...
    let enterPromptModeDisposable = commands.registerCommand('promptly.enterPromptMode', async () => {
        const editor = window.activeTextEditor;
        if (editor) {
            isPromptMode = true;
            await editor.edit(editBuilder => {
                editBuilder.insert(editor.selection.active, PROMPT_MARKER);
            });
            highlightPromptMarker(editor);
        }
    });

    let enterKeyDisposable = vscode.commands.registerCommand('type', async (args) => {
        if (args.text === '\n') {
            const editor = window.activeTextEditor;
            if (editor) {
    ...
    context.subscriptions.push(
        startChatDisposable,
        extractCodeDisposable,
        enterPromptModeDisposable,
        enterKeyDisposable,
        switchModelDisposable,
        startChatNotebookDisposable,
        handleTracebackErrorDisposable,
        setupLocalModelDisposable,
        enterCommandModeDisposable,
    ...
    if (args[0]?.toLowerCase() === 'server') {
        ...
        if (stats) {
            vscode.window.showInformationMessage(
                `Server Stats: CPU: ${stats.cpu_percent.toFixed(2)}%, ` +
...
interface ResponsePart {
	...
	const stopAnimation = async () => {
	  ...
	  await editor.edit(editBuilder => {
		const rangeToDelete = new vscode.Range(
		  ...
		  new vscode.Position(animationStartPosition.line, animationStartPosition.character + 3)
		);
	...
	interval = setInterval(() => {
	  editor.edit(editBuilder => {
		...
		const dotRange = new vscode.Range(
		  animationStartPosition,
		  new vscode.Position(animationStartPosition.line, animationStartPosition.character + dotCount)
		);
  ...
  async function handleNotebookDocumentChange(e: vscode.NotebookDocumentChangeEvent) {
	...
	const hasExecutionChanges = e.cellChanges.some(change => 
	  change.executionSummary !== undefined
	);
  ...
  async function handleChatNotebook(notebook?: vscode.NotebookDocument, errorCell?: vscode.NotebookCell, errorText?: string) {
	...
	const apiKey = vscode.workspace.getConfiguration().get('promptly.apiKey') as string;
	if (!apiKey) {
	  vscode.window.showErrorMessage('Gemini API key not configured. Please set `promptly.apiKey` in your settings.');
	  return;
	}
  ...
  async function getNormalCellSelection(notebookEditor: vscode.NotebookEditor): Promise<string | undefined> {
	const selectedCells = notebookEditor.selections.flatMap(selection => 
	  notebookEditor.notebook.getCells(selection).filter((cell: vscode.NotebookCell) => cell.kind === vscode.NotebookCellKind.Code)
	);
  ...
  async function insertCodeBlockToNotebook(notebookEditor: NotebookEditor, codeBlock: { code: string; language: string | null }) {
	const notebook = notebookEditor.notebook;
	const insertIndex = notebook.cellCount;
...
async function sendGeminiMessage(message: string, model: string, apiKey: string): Promise<string> {
  ...
  async function appendResponseToFile(editor: vscode.TextEditor, response: string, animationControl: AnimationControl, fileType: string) {
	...
	if (needsClosingTag) {
	  ...
	  const startMarkerRange = new vscode.Range(
		new vscode.Position(currentPosition.line - 1, 0),
		new vscode.Position(currentPosition.line - 1, 4 + RESPONSE_START_MARKER.length)
	  );
	  ...
	  const endMarkerRange = new vscode.Range(
		new vscode.Position(currentPosition.line + 2, 0),
		new vscode.Position(currentPosition.line + 2, RESPONSE_END_MARKER.length + 5)
	  );
	  editor.setDecorations(blueMarkerDecoration, [endMarkerRange]);
  
	  // Apply white color to the response text
	  const responseRange = new vscode.Range(
		new vscode.Position(startMarkerRange.end.line + 1, 0),
		new vscode.Position(endMarkerRange.start.line, 0)
	  );
	  ...
	  const startMarkerRange = new vscode.Range(
		new vscode.Position(currentPosition.line - 1, 0),
		new vscode.Position(currentPosition.line - 1, commentChar.length + 1 + RESPONSE_START_MARKER.length)
	  );
	  editor.setDecorations(blueMarkerDecoration, [startMarkerRange]);
	  ...
	  const endMarkerRange = new vscode.Range(
		new vscode.Position(currentPosition.line + 1, 0),
		new vscode.Position(currentPosition.line + 1, commentChar.length + 1 + RESPONSE_END_MARKER.length)
	  );
	  editor.setDecorations(blueMarkerDecoration, [endMarkerRange]);
	...
	console.log('Response appended successfully');
  }
  ...
  async function extractCodeFromLastResponse() {
	...
	lastResponseRange = new Range(
	  document.positionAt(lastResponseStartIndex),
	  document.positionAt(lastResponseEndIndex + RESPONSE_END_MARKER.length)
	);
  ...
  async function insertCodeBlock(editor: TextEditor, codeBlock: { code: string; language: string | null }) {
...
function findInsertPosition(editor: TextEditor): Position {
  ...
  function extractCodeAndLanguage(codeBlock: string): { code: string; language: string | null } {
